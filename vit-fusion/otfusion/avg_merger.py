...
class FedAvgMerger(ModelMergerBase):
    ...
    def fisher_weighted_average(self, all_params, model_coeffs, fisher_weights):
        ...
        for n, params in all_params.items():
            ...
            if 'embedding' in n or 'classifier' in n:
                avg_params[n] = params.mean(0)
                continue
            fisher = (
    ...
    def get_fisher_norm_coeff(self, all_params, fisher_weights):
        ...
        for n, _ in all_params.items():
            if n not in fisher_weights[0].keys():
                continue
            fisher_norm = torch.stack([x[n] for x in fisher_weights])
            dims = [_ for _ in range(fisher_norm.dim() - 1, 0, -1)]
            # fisher_norm = torch.norm(fisher, dim=dims)  # [N]
            for dim in dims:
                fisher_norm = torch.norm(fisher_norm, dim=dim)
            norms[n] = fisher_norm
