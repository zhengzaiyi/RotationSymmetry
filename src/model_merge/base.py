...
from .pi_merger import PermutationInvarianMerger
...
class ModelMergeExp:
    ...
    def create_merger(self):
        ...
        elif self.config.merger.algo == "pi":
            merger = PermutationInvarianMerger(
                self.config,
                self.config.merger,
                self.local_models,
                self.global_model,
    ...
    def single_round(self, save_results=True, merger_options=None):
        ...
        if save_results:
            print(f'Save results to {os.path.join(self.config.main_output_dir, "metrics.json")}')
            with open(
